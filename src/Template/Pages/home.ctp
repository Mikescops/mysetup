    ...
    <?php foreach ($fsetups as $fsetup): ?>
        ...
        <div class="slider-item">
            ...
            <a class="slider-item-inner featured-user" href="<?=$this->Url->build('/users/'.$fsetup->user_id)?>">
                <img alt="<?= __('Profile picture of') ?> <?= $fsetup->user->name ?>" src="<?= $this->Url->build('/uploads/files/pics/profile_picture_' . $fsetup->user_id . '.png?' . $this->Time->format($setup->user->modificationDate, 'Hs', null, null)); ?>">
            </a>
