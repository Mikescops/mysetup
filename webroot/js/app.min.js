...
var timer;function searchItem(query,region,action){clearTimeout(timer);timer=setTimeout(function validate(){$.ajax({url:webRootJs+'amazon/index.php',type:'get',data:{"q":query,"lang":region},success:function(response){$(".search_results."+action).html("");var el=$('<div></div>');el.html(response);var items=$('item',el);$.each(items,function(key,value){var list=$('<li></li>');var mediumimage=$('mediumimage',value);var attributes=$('itemattributes',value);var img=$('<img>');var src=$('url',mediumimage).html();img.attr('src',src);var title=$('title',attributes).html();img.attr('title',title);if(title.length>48){title=title.substring(0,48)+'..'}
