$(window).scroll(function(){var scrollTop=$(window).scrollTop();if(scrollTop<50){height=60;margin=10;$('.heavy-nav').stop().removeClass('nav-color').addClass('nav-transparent')}else{height=40;margin=0;$('.heavy-nav').stop().removeClass('nav-transparent').addClass('nav-color')}
heavyheight=height+20;$('.heavy-nav .row:first-child').stop().animate({'height':height+"px"},300);$('.heavy-nav .ms-logo').stop().animate({'height':height+"px"},300);$('.right-nav').stop().animate({'margin-top':margin+"px"},300);$('.mobile-nav').stop().animate({'margin-top':margin+"px"},300)})
$(function(){var scrollTop=$(window).scrollTop();if(scrollTop<50){height=60;margin=10;$('.heavy-nav').addClass('nav-transparent')}else{height=40;margin=0;$('.heavy-nav').addClass('nav-color')}
heavyheight=height+20;$('.heavy-nav .row:first-child').stop().animate({'height':height+"px"},100);$('.heavy-nav .ms-logo').stop().animate({'height':height+"px"},100);$('.right-nav').stop().animate({'margin-top':margin+"px"},100);$('.mobile-nav').stop().animate({'margin-top':margin+"px"},100)})
...
function doesLike(setup){$.ajax({url:webRootJs+"app/doesLike",data:{setup_id:setup},dataType:'html',type:'get',success:function(json){if(json=='true')
...
function infiniteScroll(nbtodisplay){var offset=nbtodisplay;$(window).data('ajaxready',!0);$(window).scroll(function(){if($(window).data('ajaxready')==!1)return;if(($(window).scrollTop()+$(window).height())+250>$(document).height()){$(window).data('ajaxready',!1);$.ajax({url:webRootJs+"app/getSetups",data:{p:offset,n:nbtodisplay},dataType:'html',type:'get',success:function(json){setups=$.parseJSON(json);if(setups[0]){$.each(setups,function(key,value){var nblikes;if(value.likes[0]){nblikes=value.likes[0].total}
else{nblikes=0}
console.log(value.user.modificationDate);$('.fullitem_holder').append('<div class="fullitem"><a href="'+webRootJs+'setups/'+value.id+'-'+convertToSlug(value.title)+'"><img src="'+webRootJs+value.resources[0].src+'"><\/a><div class="red_like"><i class="fa fa-heart"><\/i> '+nblikes+'<\/div><div class="fullitem-inner"><div class="row"><div class="column column-75"><a class="featured-user" href="'+webRootJs+'users/'+value.user_id+'"><img src="'+webRootJs+'uploads/files/pics/profile_picture_'+value.user_id+'.png?'+("0"+(new Date(value.user.modificationDate)).getHours()).slice(-2)+("0"+(new Date(value.user.modificationDate)).getSeconds()).slice(-2)+'"><\/a><a href="'+webRootJs+'setups/'+value.id+'-'+convertToSlug(value.title)+'"><h3>'+value.title+'<\/h3><\/a><\/div><\/div><\/div><\/div>')});$(window).data('ajaxready',!0)}
...
function checknotification(){$.ajax({url:webRootJs+"app/getNotifications",data:{n:'8'},dataType:'html',type:'get',success:function(json){notifs=$.parseJSON(json);$('#notif-container').html('');if(notifs[0]){$.each(notifs,function(key,value){$('#notif-container').append('<div onclick="markasread('+value.id+')" class="notif notifnb-'+value.id+'">'+value.content+'<div class="notif-close"><span onclick="markasread('+value.id+')">×</span></div></div>')});$('#notifications-trigger').addClass('notif-trigger');$('#no-notif').hide();instance.update(popper)}
