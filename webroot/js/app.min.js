...
function replaceValidationUI(form){form.addEventListener("invalid",function(event){event.preventDefault()},!0);form.addEventListener("submit",function(event){if(!this.checkValidity()){event.preventDefault()}});var submitButton=form.querySelector("input[type=submit]");submitButton.addEventListener("click",function(event){var invalidFields=form.querySelectorAll(":invalid"),listHtml="",label;for(var i=1;i<invalidFields.length;i++){label=form.querySelector("label[for="+invalidFields[i].id+"]");listHtml+="<li>"+label.innerHTML+" | "+invalidFields[i].validationMessage+"</li>"}
toast.alert(listHtml);if(invalidFields.length>0){invalidFields[1].focus()}})}
