window.addEventListener("load",function(){window.cookieconsent.initialise({"palette":{"popup":{"background":"#000"},"button":{"background":"#328fea"}},"theme":"classic","position":"bottom-left","content":{"href":"https://mysetup.co/pages/legals"}})});$(window).scroll(function(){var scrollTop=$(window).scrollTop();if(scrollTop<50){$('.heavy-nav').stop().removeClass('nav-color').addClass('nav-transparent')}else{$('.heavy-nav').stop().removeClass('nav-transparent').addClass('nav-color')}})
...
function galleryPreview_edit(uploader,number){if(uploader.files&&uploader.files[0]){$('#gallery'+number+'image_preview_edit').attr('style','background-image:url('+window.URL.createObjectURL(uploader.files[0])+')');$('#gallery'+number+'image_preview_edit .fa-plus').remove()}}
function galleryPreview_add(uploader,number){if(uploader.files&&uploader.files[0]){$('#gallery'+number+'image_preview_add').attr('style','background-image:url('+window.URL.createObjectURL(uploader.files[0])+')');$('#gallery'+number+'image_preview_add .fa-plus').remove()}}
...
function checknotification(){if(inIframe()==!1){$.ajax({url:webRootJs+"notifications/getNotifications",data:{n:'8'},dataType:'html',type:'get',success:function(json){notifs=$.parseJSON(json);$('#notif-container').html('');if(notifs[0]!=null){$.each(notifs,function(key,value){$('#notif-container').append('<div onclick="markasread('+value.id+')" class="notif notifnb-'+value.id+'">'+value.content+'<div class="notif-close"><span onclick="markasread('+value.id+')">×</span></div></div>')});$('#notifications-trigger').addClass('notif-trigger');$('#no-notif').hide();instance.update(notificationcenter)}
else{$('#notifications-trigger').removeClass('notif-trigger');$('#no-notif').show();instance.update(notificationcenter)}}});setTimeout(function(){checknotification()},20000)}}
function markasread(id){$.ajax({url:webRootJs+'notifications/markAsRead',type:'get',data:{"notification_id":id},});$('.notifnb-'+id).remove();if(!$.trim($('#notif-container').html()).length){$('#notifications-trigger').removeClass('notif-trigger');$('#no-notif').show();instance.update(notificationcenter)}}
...
function replaceValidationUI(form){form.addEventListener("invalid",function(event){event.preventDefault()},!0);form.addEventListener("submit",function(event){if(!this.checkValidity()){event.preventDefault()}
else{$('#publish-add').replaceWith("<span class='float-right button'><i class='fa fa-circle-o-notch fa-spin fa-fw'></i></span>")}});var submitButton=form.querySelector("input[type=submit]");submitButton.addEventListener("click",function(event){var invalidFields=form.querySelectorAll(":invalid"),listHtml="",label;for(var i=1;i<invalidFields.length;i++){label=form.querySelector("label[for="+invalidFields[i].id+"]");listHtml+="<li>"+label.innerHTML+" | "+invalidFields[i].validationMessage+"</li>"}
if(invalidFields.length>0){toast.alert(listHtml);invalidFields[1].focus()}})}
var forms=document.querySelectorAll("#add_setup_modal form");for(var i=0;i<forms.length;i++){replaceValidationUI(forms[i])}
!function(t,n){t.pageTitleNotification=function(){var e={currentTitle:null,interval:null};return{on:function(i,l){e.interval||(e.currentTitle=n.title,e.interval=t.setInterval(function(){n.title=e.currentTitle===n.title?i:e.currentTitle},l||1e3))},off:function(){t.clearInterval(e.interval),e.interval=null,n.title=e.currentTitle}}}()}(window,document);var callnotif=!1;$(window).focus(function(){if(callnotif){pageTitleNotification.off();callnotif=!1}})